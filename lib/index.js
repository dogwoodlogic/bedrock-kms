...
const ed25519sig2018 = require('ed25519-signature-2018-context');
const ed25519sig2020 = require('ed25519-signature-2020-context');
const x25519ka2020 = require('x25519-key-agreement-2020-context');

require('bedrock-did-context');
require('bedrock-veres-one-context');

jsonLdDocumentLoader.addStatic(ed25519sig2018.constants.CONTEXT_URL,
  ed25519sig2018.contexts.get(ed25519sig2018.constants.CONTEXT_URL));

jsonLdDocumentLoader.addStatic(ed25519sig2020.constants.CONTEXT_URL,
  ed25519sig2020.contexts.get(ed25519sig2020.constants.CONTEXT_URL));

jsonLdDocumentLoader.addStatic(x25519ka2020.constants.CONTEXT_URL,
  x25519ka2020.contexts.get(x25519ka2020.constants.CONTEXT_URL));
...
resolver.use(require('@digitalbazaar/did-method-key').driver());

// TODO: Move v1 'test' mode param to config.
resolver.use(require('did-veres-one').driver({mode: 'test'}));
...
const defaultDocumentLoader = async url => {
  ...
  if(url.startsWith('did:')) {
    document = await resolver.get({did: url, forceConstruct: true});
    // FIXME
  ...
  return jsonLdDocumentLoader(url);
};
